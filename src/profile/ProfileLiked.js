  ...
  useEffect(() => {
    const fetchLikedProducts = async () => {
      try {
        ...
        const response = await axios.get('https://192.168.100.90:8000/api/liked-products/', {
          params: {
            ...
            <div key={index}>
              <Link to={product.name}>
                <button className="product-buttons">
                  <img className="products-img" src={`https://192.168.100.90:8000${product.image}`} alt={product.name} />
